...
DUK_INTERNAL duk_tval *duk_create_gap(duk_context *ctx, duk_idx_t idx_base, duk_idx_t count) {
	 ...
	 * value stack.
	 */
	DUK_ASSERT((duk_size_t) (thr->valstack_end - thr->valstack_top) >= count);

	tv_src = thr->valstack_bottom + idx_base;
	tv_dst = tv_src + count;
	size = (duk_size_t) ((duk_uint8_t *) thr->valstack_top - (duk_uint8_t *) tv_src);
	DUK_MEMMOVE((void *) tv_dst, (const void *) tv_src, size);
